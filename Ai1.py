...
def get_llm_summary(ticker, price, rsi, sentiment, xgb_prob, shap_contrib, headlines):
    try:
        if "GROQ_API_KEY" not in st.secrets:
            return "⚠️ **Groq API Key missing.** Add `GROQ_API_KEY` to Streamlit Secrets to enable LLM Advisor."
        ...
        Advanced XGBoost Prediction Probability (Boost in 3 days): {xgb_prob:.1f}%.
        ...
        Keep it concise, intelligent, and professional. Use markdown bolding.
        """
        ...
        return f"Advisor service error: {e}"

# --- SIMULATION ROUTINES ---
def run_monte_carlo(df, days=30, sims=500):
...
if st.button("🚀 EXECUTE QUANTITATIVE COMMAND", type="primary", use_container_width=True):
    with st.spinner(f"Initiating Neural Models and Simulations for {selected_ticker}..."):
        ...
        if df is None or len(df) < 50: 
            st.error("Execution failure: Insufficient market flow data for this symbol.")
            st.stop()
        ...
        llm_advice = get_llm_summary(selected_ticker, latest_price, latest_rsi, avg_sentiment, xgb_prob, shap_contrib, headlines)
        ...
        m1.markdown(f'<div class="metric-card"><div class="metric-label">Live Price</div><div class="metric-value">${latest_price:.2f} ({change:+.2f}%)</div></div>', unsafe_allow_html=True)
        m2.markdown(f'<div class="metric-card"><div class="metric-label">RSI (14-Day)</div><div class="metric-value">{latest_rsi:.1f}</div></div>', unsafe_allow_html=True)
        ...
        banner_text = "🟢 GO SIGNAL (GOOD TIME TO BUY)" if xgb_prob >= 60.0 else ("⚖️ NEUTRAL (HOLD / WAIT)" if xgb_prob > 40.0 else "🔴 NO-GO SIGNAL (RISK OF DROP / SELL)")
        st.markdown(f'<h3 style="text-align:center; color:{banner_color}; background:{banner_color}1A; padding:12px; border-radius:10px; margin-top:10px;">{banner_text} — {xgb_prob:.1f}% AI Confidence</h3>', unsafe_allow_html=True)
        ...
        with tab_sum:
            ...
            with col_l:
                st.subheader("🤖 LLM Executive Advisor Summary")
                st.markdown(llm_advice)
            with col_r:
                st.subheader("📰 Recent Global Headlines")
                for h in headlines[:4]:
                    st.caption(f"• {h}")
                    st.markdown("---")

        with tab_sim:
            st.subheader(f"🔮 Stochastic Monte Carlo Cone ({days_to_sim}-Day Price Projection)")
            sim_df = pd.DataFrame(sim_paths)
            
            p10 = (sim_df.iloc[-1].quantile(0.10) - latest_price) / latest_price * 100
            p50 = (sim_df.iloc[-1].quantile(0.50) - latest_price) / latest_price * 100
            p90 = (sim_df.iloc[-1].quantile(0.90) - latest_price) / latest_price * 100

            s1, s2, s3 = st.columns(3)
            s1.metric("Expected Outcome (Median)", f"{p50:+.1f}%")
            s2.metric("Worst Case (10th Percentile)", f"{p10:+.1f}%")
            s3.metric("Best Case (90th Percentile)", f"{p90:+.1f}%")

            st.line_chart(sim_paths[:, :20])

        with tab_shap:
            st.subheader("🧬 SHAP Feature Attribution")
            st.write("This breakdown shows how much each indicator influenced the AI's final Go/No-Go verdict.")
            ...
            c = alt.Chart(shap_df).mark_bar().encode(
                x='Contribution:Q',
